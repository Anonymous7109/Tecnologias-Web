...
function pc_move(){
	...
	if((algorithm == 0 && dificulty == 1) || dificulty == 0){	//random algorithm
		...
		for (j=circles[columnToRemove].length-1 ; j >= orderToRemove ; j--) {  // j starts on the top of the column and will clean until dont find the clicked position
...
function send_configs(){
	...
	for(var i=0 ; i < numOfColumns ; i++){ // se tentar ir as settings primeiro antes de começar um jogo dá erro
		for (var j = 0 ; j < i+1 ; j++) {
			if(!(circles[i][j] == null)){  
